...
def create_app(database_uri="", test_config=None):
    ...
    def paginate_questions(request, current_questions=None):
        ...
        if current_questions:
            ...
            current_questions = Question.query.order_by(Question.id).paginate(
    ...
    def add_new_questions():
        try:
            ...
            return jsonify(
                {"success": True, "total_questions": len(Question.query.all())}
            )
        except Exception:
            abort(422)
    ...
    def add_new_category():
        try:
            ...
            db.session.commit()
            return jsonify(
                {"success": True, "total_categories": len(Category.query.all())}
            )
        except:
            abort(422)
    ...
    def delete_the_question(question_id):
        try:
            ...
            check_question_exists = Question.query.filter(
    ...
    def search_questions():
        try:
            ...
            search_question = Question.query.filter(
                func.lower(Question.question).contains(search_term)
            ).all()
    ...
    def play_game():
        ...
        try:
            ...
            if (
                int(get_quiz_category["id"]) not in allCategories.keys()
                and int(get_quiz_category["id"]) != 0
            ):
                abort(422)
            ...
            randomized_question = quiz_questions[
                random.randint(0, len(quiz_questions) - 1)
            ]
            return jsonify({"success": True, "question": randomized_question})
        except:
            abort(422)
    ...
    def unprocessable_error(error):
        return (
            jsonify({"success": False, "error": 422, "message": "Unprocessable error"}),
            422,
        )
